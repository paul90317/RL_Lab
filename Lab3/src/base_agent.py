...
class PPOBaseAgent(ABC):
	...
	def train(self):
		...
		while self.total_time_step <= self.training_steps:
			...
			while True:
				...
				action, value, action_prob = self.decide_agent_actions(observation)
				...
				obs = {}
				obs["observation_2d"] = observation[0]
				self.gae_replay_buffer.append(0, {
						"observation": obs,    # shape = (4,84,84)
						"action": [int(action)],      # shape = ()
						"reward": reward,      # shape = ()
						"value": float(value),        # shape = ()
						"logp_pi": float(action_prob),    # shape = ()
	...
	def evaluate(self):
		...
		for i in range(self.eval_episode):
			observation, info = self.test_env.reset(seed=20+i)
			total_reward = 0
			while True:
				# self.test_env.render()
				action, _, _ = self.decide_agent_actions(transform(observation), eval=True)
