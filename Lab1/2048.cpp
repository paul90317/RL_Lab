...
public:
	...
	virtual float estimate(const board& b) const {
		...
		for(int i = 0; i < iso_last; i++){
			sum += weight[indexof(isomorphic[i], b)];
		}
		return sum;
	}
	...
	virtual float update(const board& b, float u) {
		...
		float sum = 0;
		for(int i=0;i<iso_last;i++){
			sum += (weight[indexof(isomorphic[i],b)] += u);
		}
		return sum;
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				int n_possible_board = 0;
				float E_possible_board = 0;
				board after_state = move->after_state();

				// for all posible popup
				for(int i = 0; i < 16; i++){
					if(after_state.at(i) == 0) {
						after_state.set(i,1); // popup 2
						E_possible_board += estimate(after_state) * 0.9;
						after_state.set(i,2); // popup 4
						E_possible_board += estimate(after_state) * 0.1;
						after_state.set(i,0); // restore
						++n_possible_board;
					}
				}
				move->set_value(move->reward() + E_possible_board / n_possible_board);

				if (!best->is_valid() || move->value() > best->value())
					best = move;
			} else {
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for(int i = path.size() - 2; i >= 0; --i){
			target = path[i].reward() + update(path[i].before_state(), alpha * (target - estimate(path[i].before_state())));
		}
	}
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			std::ofstream ofile("log.txt", std::ios::out | std::ios::app);
			save("trained.bin");
			ofile << mean << "\n";
			ofile.close();
